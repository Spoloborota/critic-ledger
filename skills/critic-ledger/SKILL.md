...
4. **Every finding reaches a terminal status through a fix-ledger** — a
   ...
   seven-level scale of `references/readiness-scale.md`. Two older row forms stay readable and are still
   recounted: the eight-cell one without `zone`, and the seven-cell one
   without `zone` and `criterion`; the ledger header's `Row schema: v1 | v2 |
   v3` field says which form a given ledger takes. Range rows are banned (one id per row). Ledger arithmetic
   ...
   `fix`, `verified` and `terminal` are set with `templates/set-cell.py`
...
- `references/observability.md` — what the optional trace measures, the privacy invariant, and the `n/a` rule for a missing number
...
- `references/templates-and-scripts.md` — the full description of every template and script under `templates/`
...
Stage 0 reads the variable and records `subagent-model:` in the
prerequisite line.
...
When the flag line reads exactly `true`, read `${CLAUDE_PLUGIN_ROOT}/skills/critic-ledger/references/observability.md` before stage 1 and follow it.

## Stage machine

Only the first part of this router survives a context compaction; after a compaction the orchestrator re-reads the file of the CURRENT stage — the `references/` file its index entry names — before any further action, and re-reads this router in full when it cannot tell which stage it is in. With observability on, its first action there is closing any owner-wait span left open in `trace.jsonl` on the owner's word.
...
- `templates/recount.py` — the reference recount script: the contract of ledger cell values
- `templates/trace.py` — the round's span writer and validator, used only when observability is on
- `templates/rollup.py` — the stage-9 rollup, used only when observability is on
- `templates/copy-project.sh` — makes the scratchpad copy the critics read
- `templates/validate-report.py` — the deterministic half of report acceptance
- `templates/transcribe.py` — mechanical transcription of the salvaged reports into ledger rows before adjudication
- `templates/set-cell.py` — the canonical writer of a row's `fix`, `verified` and `terminal` cells
- `templates/cleanup-scratchpad.py` — removes the critics' copies and nothing else
- `templates/deleted-lines.py` — the deterministic deleted-line pre-pass for verification
- `templates/check-frontmatter.py` — inventories a SKILL.md's frontmatter against the portable Agent Skills specification
