...
<!-- Every comment block in this file is an instruction to whoever fills the
     ledger in, and comments are stripped by the script that instantiates
     this file — scripts/ledger_md.py new — so a round's own ledger
     carries the skeleton and none of this. -->
...
<!-- There is deliberately NO lens table and no token column: a second
     machine-readable copy of the same facts is a second source of truth,
     and the one that drifts is always the hand-maintained one. This file
     measures QUALITY — what was found, what was fixed, what was
     verified. -->

- Ledger state: {open | closed <ISO-date> | FROZEN (superseded-by-rewrite, date + reason)}.
...
- Object: full path {absolute paths or commit range} | short name {object-slug}. Pin: {commit hash}.
  <!-- The short name is only the run-folder address, the full
  path lives here. The pin: git is mandatory, so the pin is
...
- Mode: {plan | impl}.
- Profile: {auto | F | M | L}
  <!-- `auto` at stage 2, where the header is
  filled whole; the orchestrator overwrites it with the letter at the close
  of stage 6, from the recount's upheld count or from the contract's fixed
  value; no script reads or writes this line. -->
- Prerequisite (stage 0): git = {yes/no}, commit sanction = {yes/no}; {normal | DEGRADED} mode; subagent-model: {set <value> | set <non-model-value> | unset}.
- Round contract: {path of the signed contract file} sha {sha of that file at the round's start} | none — object under the 400-line gate, owner waived it on {date}.
  <!-- The contract is a DURABLE file beside the object, from
...
- Contract amendments: {none | Amendment {n}: {ISO date} — {one sentence on what it changes}, one entry per amendment}.
  <!-- An amendment made INSIDE the round is
...
- Round-started: {ISO-8601 UTC | n/a}
  <!-- — when stage 1 created this file. It
...
- Lenses:
  ...
  <!-- These dashed lines directly under the field, one per lens and NOTHING
       ...
       the recount id contract). scripts/recount.py reads `k` — the lens count
       ...
       lens, `owner-set` for a lens the owner asked for, and a note on a lens
       dropped because a respawn failed. -->
- Verifier passes: {verifier-prefix stem | n/a}.
  <!-- Stage 2 fixes ONE stem for
...
- Process prefixes: none
  <!-- Replace `none` on the line above — and nothing else on it, the whole
       ...
       lens — it is read by scripts/recount.py through its own pattern, it
...
- Security lens: none
  ...
  arms a gate, and scripts/recount.py refuses a value it cannot read, so an
  unfilled placeholder would fail every round that has no security lens at all. -->
- Load-bearing claims of the object (why this many lenses): {one line per claim whose falsity would make the object unfit}.
  <!-- The lens count equals
  the length of this list — the list is what justifies the count, and
  nothing checks the two against each other; `owner-set` lenses come ON TOP
  of it. -->
- Git history requested by: {lenses that declared they need it | none}; copy is {per-critic | shared for the round}.
  <!-- With a shared copy the
  history one lens asked for is readable by all — accepted knowingly, not
  hidden. -->
- Excluded from the critics' copy: {classes of files withheld — env and secret patterns always, plus whatever was dropped for size when the copy had to be narrowed}.
  <!-- A critic must know what it never saw, or its
  coverage statement lies; silent narrowing is banned. -->
- Concurrent critics: {actual sum = lenses + repeat passes on critical claims + owner-set lenses} against the hard budget of 12.
  <!-- The sum is checked BEFORE
  spawning. The same number is the count of simultaneous clones of the
  private tree. -->
- Batch-commit executor: {orchestrator | project convention (name it) | none (degraded)}.
- Salvages: {paths of the verbatim report files, under {run-folder}/critics/ and {run-folder}/verify/}. Privacy confirmed for this repository BEFORE the first salvage: {yes | local unversioned holding used instead}.
- Precedents file: {{run-folder}/precedents.md | not created yet — stage 6 not reached}.
  <!-- Created and kept by the ORCHESTRATOR only, at the first
...
- Scratchpad not cleaned: {reason, path | cleaned {date}}.
  ...
  reason lands here. The field is ONE line — the reason and the path — and
  never holds the dry-run output: that output, which named what was to be
  deleted, is kept beside this ledger in the run folder's
  `scratchpad-cleanup-plan.txt`. That output, like the path this field
  names, is ABSOLUTE and carries the machine's scratchpad root, and this
  header and the run folder may be versioned as evidence, so both pass the project's leak/PII
  sanitization discipline first, fail-closed: where that pass cannot be
  run, the reason is recorded here and the path and the output are not. -->
- Row schema: v3
  <!-- The value is the WHOLE remainder of the line above, exactly as with
  `Security lens:` above, and it must be one of `v1` / `v2` / `v3` — the
  form the findings table takes, declared once so that
  scripts/recount.py, scripts/transcribe.py and the verifier know what
  ...
  belongs in this comment, never on the field's own line. -->
- Zone map: {path of the round contract § "Zone map" | the table itself, when the object was under the 400-line gate and the contract was pasted into this header}.
  <!-- Written by the ORCHESTRATOR at stage 2, from the contract the
...
<!-- The `zone` cell (v3 only): `Z1`, `Z2` or `Z3`, taken from the zone map
  ...
  the ADJUDICATOR at stage 6, never by transcription — scripts/transcribe.py
...
- Exit family: {clean-streak | residual-risk | residual-risk close (stop-rule freeze)}.
  <!-- Which of the two legitimate exits closed this round — the streak
...
<!-- The stop rule's freeze, when it fires, adds ONE header line here, written
  ...
  folder | pending}`. -->
  <!-- That line is ONE per ledger and is written ONCE, by
...
- Residue register: {path of the project's .critic-ledger/residue-register.md | none — nothing nominated in this project yet}.
  <!-- The register lives OUTSIDE
       this run folder on purpose: it outlives any one run. Its skeleton is
       templates/residue-register.md. -->
- Residue carried in: {n rows, oldest {age} | none}.
  <!-- Quoted from the
...
<!-- Verdict-cell tags. Optional tags live inside the `verdict` cell and
     are read by scripts/recount.py: `class:<slug>` — the defect class a
...
<!-- New-findings curve by pass: computed by scripts/recount.py from the
...
threshold of 20 CONFIRMED findings and the second pass was skipped by default. The
threshold buys tokens, not correctness: the owner may ask for the pass
anyway, and from 20 CONFIRMED findings up it is mandatory.
...
too-small union of scopes visible at a glance. The column is read by nobody
mechanically: the recount finds every column it needs BY NAME, and a column
it does not know it ignores by construction. -->
